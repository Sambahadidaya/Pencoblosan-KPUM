...
// ====================
// FETCH WAKTU AKURAT DARI API (UNTUK MENCEGAH MANIPULASI WAKTU DEVICE)
// ====================
let serverTimeOffset = 0; // Offset antara waktu server dan waktu lokal (dalam milidetik)
let isTimeFetched = false; // Flag untuk menandai apakah waktu sudah di-fetch

async function fetchServerTime() {
  try {
    // Gunakan HTTPS untuk menghindari masalah CORS
    const response = await fetch('https://worldtimeapi.org/api/timezone/Asia/Jakarta');
    if (!response.ok) throw new Error('Response not OK');
    const data = await response.json();
    const serverTime = new Date(data.datetime); // Waktu server dalam UTC, tapi API sudah dalam Jakarta timezone
    const localTime = new Date();
    serverTimeOffset = serverTime.getTime() - localTime.getTime(); // Hitung offset
    isTimeFetched = true;
    console.log('Waktu server berhasil di-fetch:', serverTime.toLocaleString());
  } catch (error) {
    console.error('Gagal fetch waktu server:', error);
    // Fallback: Gunakan waktu lokal, tapi beri peringatan dan coba lagi setelah beberapa detik
    serverTimeOffset = 0;
    isTimeFetched = true; // Tetap set true agar tidak loop, tapi gunakan lokal
    // showMessage("Gagal mendapatkan waktu akurat dari server. Menggunakan waktu device Anda. Pastikan waktu device benar!", "error");
    // Opsional: Coba lagi setelah 1 detik
    setTimeout(fetchServerTime, 100);
  }
}

// Fungsi untuk mendapatkan waktu sekarang yang akurat (dengan offset)
function getAccurateNow() {
  const localNow = new Date();
  return new Date(localNow.getTime() + serverTimeOffset);
}

// ====================
// UPDATE COUNTDOWN UNTUK HALAMAN LOGIN (DENGAN WAKTU AKURAT)
// ====================
function updateCountdown() {
  if (!isTimeFetched) return; // Tunggu hingga waktu di-fetch

  const now = getAccurateNow(); // Gunakan waktu akurat
  const countdownLabel = document.getElementById('countdownLabel');
  const countdownContainer = document.getElementById('countdownContainer');

  if (now < startTime) {
    // Countdown ke pembukaan
    ...
    if (countdownLabel) countdownLabel.innerText = 'Waktu Menuju Pembukaan Voting';
    if (countdownContainer) countdownContainer.style.display = 'block';
  } else if (now < endTime) {
    // Countdown ke penutupan
    ...
    if (countdownLabel) countdownLabel.innerText = 'Waktu Menuju Penutupan Voting';
    if (countdownContainer) countdownContainer.style.display = 'block';
  } else {
    // Sudah melewati penutupan
    if (countdownLabel) countdownLabel.innerText = 'Waktu Voting Sudah Habis';
    if (countdownContainer) countdownContainer.style.display = 'none'; // Sembunyikan box countdown
  }

  // Update tombol login setiap detik berdasarkan waktu akurat
  updateLoginButton();
}
...
// ====================
const startTime = new Date('2025-12-01T00:00:00+07:00'); // Waktu mulai: 1 Desember 2025, 08:00 WIB
...
function isVotingOpen() {
  if (!isTimeFetched) return false; // Jika waktu belum di-fetch, anggap belum terbuka
  const now = getAccurateNow();
  return now >= startTime && now <= endTime;
}

// Fungsi untuk menghitung waktu tunggu hingga mulai (format: jam-menit) (DENGAN WAKTU AKURAT)
function getTimeUntilStart() {
  if (!isTimeFetched) return null;
  const now = getAccurateNow();
  if (now < startTime) {
    const diff = startTime - now; // Selisih dalam milidetik
...
// Fungsi untuk update status tombol login berdasarkan waktu (BARU)
function updateLoginButton() {
  ...
  if (isVotingOpen()) {
    ...
    loginBtn.innerHTML = '<span class="spinner hidden"></span>Masuk ke Voting'; // Reset ke teks asli
  } else {
    loginBtn.disabled = true;
    if (!isTimeFetched) {
      loginBtn.innerHTML = '<span class="spinner hidden"></span>Memuat Waktu...';
    } else if (getAccurateNow() < startTime) {
      loginBtn.innerHTML = '<span class="spinner hidden"></span>Voting Belum Dibuka';
    } else {
      loginBtn.innerHTML = '<span class="spinner hidden"></span>Voting Sudah Selesai';
    }
  }
}
...
document.addEventListener('DOMContentLoaded', async () => {
  // Fetch waktu server terlebih dahulu
...
// ====================
// LOGIN PEMILIH (DENGAN VALIDASI WAKTU AKURAT)
// ====================
...
if (voterForm) {
  voterForm.addEventListener("submit", async function (e) {
    ...
    // Pengecekan waktu sebelum proses login (BARU)
    if (!isVotingOpen()) {
      if (!isTimeFetched) {
        showMessage("Sedang memuat waktu. Coba lagi dalam beberapa detik!", "error");
      } else if (getAccurateNow() < startTime) {
        const waitTime = getTimeUntilStart();
        showMessage(`Maaf e-voting belum dibuka, Silahkan tunggu ${waitTime} lagi`, "error");
      } else {
        showMessage("Maaf waktu e-voting sudah selesai", "error");
      }
